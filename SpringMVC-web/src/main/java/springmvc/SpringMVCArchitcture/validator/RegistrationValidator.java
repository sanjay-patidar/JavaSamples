...
public class RegistrationValidator implements Validator{
	...
	public void validate(Object object, Errors errors) {
		...
		if (regmod.getEmail() != null && regmod.getEmail().trim().length() > 0) {
			...
			} else if (regmod.getEmail().contains(".com") != true
					&& regmod.getEmail().contains(".net") != true) {
				errors.rejectValue("email","error.mail", "error.please use .com or .net");
			}
		}
		
		if(regmod.getPassword().trim().length()<8 && regmod.getPassword().trim().length()>16 && regmod.getPassword().contains("-")!=true)
		{
		...
		/*if(regmod.getMobilenumber()!=0)
		{
			 ...
			 Matcher matcher = pattern.matcher(regmod.getMobilenumber());
			 if (((Matcher) matcher).matches() != true) {
				    errors.rejectValue("mobilenumber", "error.mob",
